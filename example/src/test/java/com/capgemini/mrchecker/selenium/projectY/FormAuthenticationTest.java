...
public class FormAuthenticationTest extends TheInternetBaseTest {
    ...
    private final int maxKeyboardInputAttemps = 10;
    ...
    @Test
    public void shouldSendAllKeysFromKeyboardToInputForm() {
        for (int attemptNumber = 0; attemptNumber < maxKeyboardInputAttemps; attemptNumber++) {
            logStep("Typing username and password from keyboard attempt no "+attemptNumber);
            logStep("Type username from keyboard");
            formAuthenticationPage.typeUsernameByKeyboard(validUsername);
            logStep("Type password from keyboard");
            formAuthenticationPage.typePasswordByKeyboard(validPassword);
            logStep("Trying to login");
            formAuthenticationPage.clickLoginButton();
            logStep("Verify if username has been typed properly");
            assertEquals("Unable to login user with valid credentials", loginMessage,
                    formAuthenticationPage.getLoginMessageText());
            logStep("Log out user");
            formAuthenticationPage.clickLogoutButton();
